...
public class MoaiActivity extends Activity {
    ...
    try {
		...
		try {
			...
			File dir = new File(Environment.getDataDirectory().getAbsolutePath() + "/moai_lua");
			MoaiLog.i( "Checking if directory exists " + dir.getAbsolutePath());

			if(dir.exists() && dir.isDirectory()) {
				Moai.mount ( "bundle", dir.getAbsolutePath() );
				Moai.setWorkingDirectory ( dir.getAbsolutePath() );				
				MoaiLog.i( "Setting to " + dir.getAbsolutePath());
			} else {
				Moai.mount ( "bundle", myApp.sourceDir );
				Moai.setWorkingDirectory ( "bundle/assets/@WORKING_DIR@" );
				MoaiLog.i( "Setting to " + myApp.sourceDir );
			}
		} catch ( NameNotFoundException e ) {
